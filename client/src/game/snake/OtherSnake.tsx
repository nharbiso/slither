...
export default function OtherSnake({positions, offset}: {positions: Set<Position>, offset: Position}) {
    return (
        <div>
            {Array.from(positions).map((bodyPart: Position) =>
            <div className="snake" style={{left: bodyPart.x + offset.x, top: bodyPart.y + offset.y}}></div>)}
        </div>
    )
}
